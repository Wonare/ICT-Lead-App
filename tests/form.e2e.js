...
const TEST_LEAD = {
  ...
  phone: '08098765432',
  email: 'chidinma.e2e@example.com',
...
async function main() {
  ...
  const all = await api('/api/admin/leads?limit=1000', auth);
  let removed = 0;
  for (const l of all.json.data.leads) {
    const del = await api('/api/admin/leads/' + l.id, { method: 'DELETE', headers: auth.headers });
    if (del.json && del.json.success) removed++;
  }
  console.log('  removed ' + removed + ' test lead(s) - database left clean.');
