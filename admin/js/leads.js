...
(function () {
  ...
  saveBtn.addEventListener('click', function () {
    ...
    AdminAPI.put('/api/admin/leads/' + currentLead.id, { status: newStatus, notes: newNotes })
      .then(function () {
  ...
  document.getElementById('exportBtn').addEventListener('click', function () {
    ...
    function getPage(page) {
      return AdminAPI.get('/api/admin/leads?' + buildQuery({ page: page, limit: 1000 })).then(function (json) {
        collected = collected.concat(json.data.leads);
        if (page < json.data.pagination.totalPages) return getPage(page + 1);
        return { data: { leads: collected } };
      });
    }
    getPage(1)
      .then(function (json) {
